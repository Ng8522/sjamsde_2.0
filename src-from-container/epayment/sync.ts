...
async function getLastSyncAt(): Promise<Date> {
  ...
  const { rows } = await pool.query<{ value: string }>(
    `SELECT value FROM ${table("sync_state")} WHERE key = $1`,
    [SYNC_KEY],
  );
...
async function setLastSyncAt(value: Date) {
  const pool = getPool();
  await pool.query(
    `INSERT INTO ${table("sync_state")} (key, value, updated_at)
     VALUES ($1, $2, NOW())
     ON CONFLICT (key) DO UPDATE
     SET value = EXCLUDED.value,
         updated_at = NOW()`,
    [SYNC_KEY, value.toISOString()],
  );
}

export async function fetchCompletedTransactions(since: Date): Promise<EpaymentRow[]> {
  ...
  try {
    ...
    const result = await request.query<EpaymentRow>(await readQuery());
...
export async function syncEpaymentTransactions() {
  ...
  for (const row of rows) {
    ...
    if (after.length > before.length) imported += 1;
    else skipped += 1;

    if (paidAt > latest) latest = paidAt;
  }
