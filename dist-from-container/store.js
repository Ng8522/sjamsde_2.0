...
export async function getDonationSummary() {
    ...
    const [targetRow, totals] = await Promise.all([
        pool.query(`SELECT amount FROM ${table("target")} WHERE id = 1`),
        pool.query(`SELECT
...
export async function setDonationTarget(amount) {
    if (!Number.isFinite(amount) || amount <= 0) {
        throw new Error("amount must be a positive number");
    }
    const pool = getPool();
    await pool.query(`INSERT INTO ${table("target")} (id, amount, updated_at)
     VALUES (1, $1, NOW())
     ON CONFLICT (id) DO UPDATE
     SET amount = EXCLUDED.amount,
         updated_at = NOW()`, [amount]);
    return getDonationSummary();
}
export async function listDonors() {
    const pool = getPool();
    const { rows } = await pool.query(`${DONOR_SELECT} ORDER BY created_at DESC`);
    return rows.map(rowToRecord);
}
export async function findDonorByTransactionRef(transactionRef) {
    const pool = getPool();
    const { rows } = await pool.query(`${DONOR_SELECT} WHERE transaction_ref = $1 LIMIT 1`, [transactionRef]);
...
export async function addDonor(input) {
    ...
    await pool.query(`INSERT INTO ${table("donors")} (id, donor_name, transaction_ref, amount, date_time)
     VALUES ($1, $2, $3, $4, $5)`, [id, donorName, input.transactionRef?.trim() || null, input.amount, dateTime]);
    const { rows } = await pool.query(`${DONOR_SELECT} WHERE id = $1`, [id]);
